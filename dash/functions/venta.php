...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        echo <<<HTML
        <script>
            alert("Lo siento, recargaste la página cuando la venta ya se había generado.");
            window.location.href = "../vistas/escoger-numero.php";
        </script>
        HTML;
        exit();
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $totalPagar = mysqli_real_escape_string($conexion, $_POST["total_pagar"]);

        //limpiamos el punto
        $totalPagar = str_replace('.', '', $totalPagar);

        $payment_id = "Número específico";

        $vendido_por = $_SESSION['usuario_nombre']; // Recuperar el valor de la variable de sesión 'usuario'

        $vendido_por = ucfirst($vendido_por); // Convertir la primera letra a mayúscula
        ...
        try {
            ...
            if ($resultado_numeros->num_rows >= $totalNumeros) {
                ...
                if ($consulta_venta->affected_rows > 0) {
                    ...
                    $id_venta = $consulta_venta->insert_id;
                    ...
                    $consulta_eliminar_numeros = $conexion->prepare("DELETE FROM numeros WHERE numero = ?");

                    // Array para almacenar los números vendidos
                    $numeros_vendidos = array();

                    // Iterar sobre los números seleccionados y procesarlos
                    foreach ($_POST['numeros'] as $numero) {
                        ...
                        $consulta_eliminar_numeros->bind_param("i", $numero);
...
<div class="container mt-1" style="background-color: #fff;">
    <div class="mt-2">
        ...
        <div class="text-center mt-2 mx-5">
            ...
            <a href="../vistas/escoger-numero.php" class="btn btn-sm btn-secondary">Regresar</a>
        </div>
    </div>
</div>
</div>
